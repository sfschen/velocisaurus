...
from lpt_rsd_vel_moments_fftw import LPT_RSD_VelocityMoments
...
class PairwiseVelocityLikelihood(Likelihood):
    ...
    def predict(self, thetas=None):
        ...
        tt = []
        self.kv, (self.p0, self.p2, self.p4), (self.v1,self.v3), (self.s0,self.s2) = modPT.combine_bias_terms_Xi_n(bvec)
        ...
        for spec in [self.p0, self.p2, self.p4, self.v1, self.v3, self.s0, self.s2]:
            tt += [ np.dot(self.binning_matrix, spec), ]
            #tt += [interp1d(self.kv, spec, kind='cubic')(self.kdat),]
...
class PT_Theory(Theory):
    ...
    def initialize(self):
        ...
        self.LPT_RSD = LPT_RSD_VelocityMoments(self.klin, self.plin, kIR=self.kIR)
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        fz = pp.get_param('fz')
        self.LPT_RSD.make_Xi_n_table(fz, nk=self.nk, kmin=self.kmin, kmax=self.kmax)

        state['ptmod'] = self.LPT_RSD
        #